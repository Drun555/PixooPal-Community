...
const PIECES: Record<PieceKind, Point[][]> = {
  i: [
    [
      { x: 0, y: 1 },
      { x: 1, y: 1 },
...
const COLORS: Record<PieceKind, ClockfacePixel> = {
  i: [92, 225, 230],
  j: [91, 135, 255],
  l: [255, 164, 72],
  o: [255, 223, 82],
...
let active = createPiece(randomKind());
let next = createPiece(randomKind());
let lockFrames = 0;
let score = 0;
let lines = 0;
...
function hardDrop(context: ClockfaceContext) {
...
function moveActive(dx: number, dy: number) {
...
function lockPiece(context: ClockfaceContext) {
...
function renderTetris(context: ClockfaceContext) {
...
function drawBoard(context: ClockfaceContext) {
...
function drawPiece(context: ClockfaceContext, piece: Piece, opacity: number) {
...
function drawGhost(context: ClockfaceContext) {
  let ghost = { ...active };
...
function drawCell(context: ClockfaceContext, x: number, y: number, fill: ClockfacePixel, opacity: number) {
...
function getBestPlan(): AutoPlan | null {
  let best: { plan: AutoPlan; score: number } | null = null;
  ...
  for (let rotation = 0; rotation < PIECES[active.kind].length; rotation += 1) {
    for (let x = -3; x < BOARD_WIDTH + 3; x += 1) {
      ...
      const lines = getFullRows(simulated).length;
      const score = lines * 80 - getAggregateHeight(simulated) * 2 - getHoles(simulated) * 14 - getBumpiness(simulated) * 3;
...
function canPlace(piece: Piece, targetBoard: Cell[][]) {
...
function placePiece(source: Cell[][], piece: Piece) {
...
function getFullRows(targetBoard: Cell[][]) {
  const rows: number[] = [];
...
function clearRows(source: Cell[][], rows: number[]) {
...
function getAggregateHeight(targetBoard: Cell[][]) {
  return getColumnHeights(targetBoard).reduce((sum, height) => sum + height, 0);
}

function getHoles(targetBoard: Cell[][]) {
  let holes = 0;
...
function getBumpiness(targetBoard: Cell[][]) {
...
function getColumnHeights(targetBoard: Cell[][]) {
...
function getShape(kind: PieceKind, rotation: number) {
...
function getPieceKey(piece: Piece) {
  return `${piece.kind}:${piece.rotation}:${piece.x}:${piece.y}`;
}

function normalizeResolution(value: number) {
