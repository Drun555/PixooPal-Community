import { defineClockface, type ClockfaceContext, type ClockfacePixel } from '@pixoopal/clockface';
...
type TimePalette = {
  hour: number;
  top: ClockfacePixel;
  bottom: ClockfacePixel;
  text: ClockfacePixel;
  glow: ClockfacePixel;
};

const PALETTES: TimePalette[] = [
  {
    hour: 0,
    top: [7, 10, 32],
    bottom: [20, 12, 42],
...
function renderClock(context: ClockfaceContext) {
  const now = new Date();
...
function drawBackground(context: ClockfaceContext, palette: TimePalette) {
...
function getPalette(hour: number): TimePalette {
