import { color, data, defineClockface, input, type Pixel } from '@pixoopal/clockface';
import {
  drawBitmapText,
...
const CALENDAR_REFRESH_MS = 60_000 * 1;
const CARD_X = 0;
const CARD_Y = 18;
const CARD_WIDTH = 64;
const CARD_HEIGHT = 46;
const ERROR_COLOR: Pixel = [255, 184, 168];

const calendarTemplate = String.raw`
...
async function refreshEvents(
  homeAssistant: { renderJinja(template: string, variables?: Record<string, unknown>): Promise<string> },
  ...
  try {
    ...
    const payload = JSON.parse(response.trim() || '{}') as CalendarPayload;
...
function getNextEvent(events: CalendarEvent[], now: Date) {
  ...
  return events
    ...
    .filter(({ endMs, startMs }) => Number.isFinite(startMs) && Number.isFinite(endMs) && endMs >= nowMs)
    .sort((left, right) => left.startMs - right.startMs)[0]?.event;
}

function drawEventCard(buffer: Uint8Array, size: number, fill: Pixel) {
...
async function drawMarqueeText(
  buffer: Uint8Array,
  size: number,
  text: string,
  x: number,
...
async function drawCenteredText(buffer: Uint8Array, size: number, text: string, y: number, fill: Pixel) {
  await drawText(buffer, size, text, Math.max(0, Math.floor((size - measureBitmapText(text)) / 2)), y, fill);
}

async function drawText(buffer: Uint8Array, size: number, text: string, x: number, y: number, fill: Pixel) {
  await drawBitmapText({
    buffer,
    size,
    text,
...
function drawRoundedRectOutline(
  ...
  for (let targetY = y; targetY < y + height; targetY += 1) {
    for (let targetX = x; targetX < x + width; targetX += 1) {
      if (
        isInsideRoundedRect(targetX, targetY, x, y, width, height, radius) &&
        !isInsideRoundedRect(targetX, targetY, x + 1, y + 1, width - 2, height - 2, Math.max(0, radius - 1))
      ) {
...
function setPixel(buffer: Uint8Array, size: number, x: number, y: number, fill: Pixel) {
...
function formatRelativeTime(event: CalendarEvent, now: Date) {
  if (event.allDay) {
    return 'All day';
  }

  const minutes = Math.round((getEventRange(event).startMs - now.getTime()) / 60_000);
...
function normalizeCalendarEntity(value: string) {
  const normalized = value.trim();
...
function lighten(fill: Pixel, amount: number): Pixel {
  return [
